...
from mywish_models.models import InvestRequests, session
...
class ERC20PaymentMonitor:
    ...
    def on_new_block_event(cls, block_event: BlockEvent):
        print('ERC20-MAINNET: meow', flush=True)
    ...
    def handle(cls, token_address: str, token_name, transactions, network):
        for tx in transactions:
            ...
            model = session.query(InvestRequests).\
                filter(InvestRequests.eth_address == transfer_to.lower()).first()
            if not model:
                continue
